...
## 4.15 The disinterested estimate: what the volunteers returned
...
removes both, putting sampled claims to volunteers who did not build the
tool and are not shown what it predicted. It ran, and it returned much less
than it was designed to collect: 226 usable judgements from five raters
against a sample of 2,002, covering 176 claims, with one rater supplying 53%
of them. What follows is a pilot rather than the study, and nothing below
leans on it harder than that allows.

The item set was drawn on 17 July, a month before `on_contact_min` was
re-fitted, so the volunteers judged pre-refit labels and the comparison is
against §4.14's first pack rather than the shipped second one. For the five
predicates the support threshold does not touch, the distinction does not
arise.
...
an author verdict the two agree 0.744 of the time, Cohen's κ 0.470. The
author is therefore neither rubber-stamping nor fully reproducible by a
stranger, and the fair summary is that the author's verdicts are
corroborated where they are unflattering and disputed where they turn on a
convention. Crowd-internal reliability is Krippendorff's α 0.397 across the
...
**What this arm settles and what it leaves open.** It bounds the direction
of the author's bias on the measurement that matters most, and on support it
finds that bias, if it exists at all, running against the tool rather than
for it. It does not replace the author's audit, and at five raters and 8.8%
of the designed sample it could not: the arm is reported here because a weak
disinterested estimate pointing the same way as a strong interested one is
worth more than the absence it replaces, not because it closes the question.
§7.6 and §9.3 carry the limitation that survives.
