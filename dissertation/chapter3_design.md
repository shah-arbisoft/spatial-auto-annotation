...
semi-supervised pseudo-labelling or by active learning. Section 2.4 sets out
that rival and the three properties of this dataset that argue against it,
all of which Chapter 5 then measures: the seed is internally inconsistent,
it is not a random sample of what it would be asked to extend, and it is
weakest exactly where the task is hardest, the teacher such a loop would
start from reaching 0.08–0.25 recall on the sparsely-labelled predicates.
...
- **to the left of / to the right of** compare horizontal centres in the
  ...
  the faithful choice among the three RoboSpatial distinguishes (Song et al.,
  2025). An ambiguity band (0.02) abstains and flags when centres nearly
  coincide.
- **in front of / behind** is a two-stage cascade. Depth ordering decides
  ...
  from pure projection, ordering two objects on the same floor by which box
  bottom sits lower in the image: a pixel-precise cue exactly where depth is
