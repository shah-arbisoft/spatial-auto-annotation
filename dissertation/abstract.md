...
whether the human can be removed from the labelling loop entirely, for the
seven predicates the dataset defines (on, under, left/right of, in front
of/behind, near).
...
object is lifted to a 3D position, and explicit geometric rules decide every
label. Thresholds are fitted on six of the nine annotator groups and
validated on the held-out three; outputs use the dataset's native formats.
The full 836-image dataset is annotated in about five minutes on a consumer
GPU, at 20 times the label density of the manual pass.
...
training, where the automatically trained model recalls sixty times more
(zR@100 0.172 against 0.003). Per annotator, that advantage is large against
both annotators carrying a measured labelling defect and disappears against
the only one without. The metric therefore rewards agreement with annotation
habits as well as spatial correctness, a benchmark property rather than a
label-source one, so the answer is conditional: automatic labels are the
better supervision wherever ground truth means geometry, human labels
wherever it means annotation practice. Robot planning needs the former. The
bottleneck it removes limited not only the dataset's size but what it could
teach.
