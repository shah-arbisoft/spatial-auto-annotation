...
0.96 of the time, so a predicate recovering 0.70 of the human labels is
applying an unshared criterion rather than guessing.
...
that advantage disappears without reversing: the arms rank level (mR@100
...
annotation habits as well as spatial correctness.

One link down the chain the relations decide the outcome: asked for a safe
grasp plan on 25 held-out scenes where an object rests on the target, a
...
the automatic and vision-language sources, identically on two planners of
very different capability. Automatic labels are at least the equal of human
ones where the criterion is annotation practice and better where it is
geometric consistency, and robot planning needs the second. The bottleneck
removed was limiting not only the dataset's size but what it could teach.
