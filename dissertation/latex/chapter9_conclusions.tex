...
\section{Summary: the objectives and the answers}
...
\textbf{Methodologically.} Calibration held out by \textit{annotator} rather than by image; a sparse-gold evaluation protocol that pairs recall with audited precision; exhaustive loss attribution; and a way to estimate what annotators would score against one another when they never labelled the same images, by using a deterministic annotator as a fixed common reference (\S4.6).

\section{Limitations, and the work that would resolve them}
...
\textbf{Scale is demonstrated in principle, not in practice.} Throughput and density are measured on 836 images. Applying the pipeline to genuinely new robot captures, which the supervising group can supply, is the natural next step and the one that would make the scaling claim concrete rather than extrapolated.
