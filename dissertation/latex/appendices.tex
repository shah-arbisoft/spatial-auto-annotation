...
The reproduction is the point. Mounting the dataset read-only and running \texttt{scripts/reannotate\_from\_cache.py} inside the container produces a \texttt{pairs.csv} of 84,881 rows whose SHA-256 matches the file committed here exactly, so the container does not merely install, it reproduces the annotations this dissertation reports. \texttt{outputs/docker/verification.md} records the full check.
