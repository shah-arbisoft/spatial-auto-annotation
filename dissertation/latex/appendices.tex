...
\textbf{1. Environment.} Either build the container (\texttt{docker build -t spatial-annotator .}; the \texttt{Dockerfile} at the repository root pins Python 3.11, torch 2.5.1 + cu121 and installs SAM2 from GitHub, and runs the test suite at build time; a \texttt{.dockerignore} holds the build context to the 94 source files that belong in an image, keeping the caches, the dataset and the credentials file out of it), or create a Python 3.11/3.12 venv and follow the three numbered notes at the top of \texttt{requirements.txt}. The known pitfall is documented there: installing SAM2 can silently replace CUDA torch with a CPU wheel, fixed by reinstalling torch with \texttt{--no-deps --force-reinstall} from the cu121 index. Verify with \texttt{python scripts/smoke\_test.py --image assets/sample.jpg}, which loads SAM2 and Depth Anything and reports CUDA availability and peak memory (\textasciitilde{}0.65 GB).
