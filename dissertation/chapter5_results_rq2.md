...
This chapter answers RQ2 with a controlled experiment. Section 5.1 states the
design, §5.2 the result, §5.3 the mechanism and its consistency checks,
§5.4 the boundaries of the claim, §5.5 the connection to the source paper's
robot-planning chain, and §5.6 the answer.
...
removes. The front/behind rows are depressed for both models by the
...
that removing the human bottleneck can *grow* dataset utility rather than
merely approximate it, demonstrated on the dataset's own held-out annotators.
