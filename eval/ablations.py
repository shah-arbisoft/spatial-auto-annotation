...
def main():
    ...
    md.append("| band | front recall | front (held-out) | front P(restr.) | "
              "behind recall | behind (held-out) | behind P(restr.) |")
    md.append("|---|---|---|---|---|---|---|")
    for v, s in sweep(scenes, base, "plane_band",
                      [9.9, 0.0, 0.005, 0.01, 0.02, 0.05], use_contact=True):
        f, b = s["in front of"], s["behind"]
        label = "off" if v > 1 else f"{v:.3f}"
        md.append(f"| {label} | {fmt(f['recall'])} | {fmt(f['recall_ho'])} | "
                  f"{fmt(f['precision_restricted'])} | {fmt(b['recall'])} | "
                  f"{fmt(b['recall_ho'])} | {fmt(b['precision_restricted'])} |")
    md.append("\nshipped plane_band = **0.005** - selected on train groups (added "
              "commits at 0.91 direction agreement; held-out group 7: all added "
              "commits correct). The fallback fires only when the depth rule "
              "abstained and neither object rests on another by the tool's own "
              "contact evidence; without masks it is off (A2 is the pure-depth "
              "trade for reference).\n")

    Path("outputs/tables/ablations.md").write_text("\n".join(md), encoding="utf-8")
    print("\n".join(md))
    # Figures are generated by scripts/make_figures.py, which reads this table
    # (the near_T sweep among them) so every figure shares one house style.
