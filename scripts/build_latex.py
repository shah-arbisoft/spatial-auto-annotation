...
# Body baselines a row occupies at each size, and the mean glyph advance
# there, for estimating how tall a table will come out.
SIZE_LINE = {r"\small": 11.0 / 12.0, r"\footnotesize": 10.0 / 12.0,
             r"\scriptsize": 8.0 / 12.0}
SIZE_CHAR = {r"\small": CHAR_PT, r"\footnotesize": CHAR_PT * 10.0 / 11.0,
             r"\scriptsize": CHAR_PT * 8.0 / 11.0}

# A table shorter than this is required to sit on one page; anything longer
# cannot, so it is left to break where longtable chooses. The a4 text block
# holds about 46 single-spaced 12pt lines, so a table reserving 36 still has
# room to land whole on the page it is pushed to.
KEEP_WHOLE_LINES = 36.0


def table_height(cells: list[list[str]], spec: str, size: str,
                 caption: str) -> float:
    """Roughly how many body baselines the rendered table will occupy.

    Only the decision "does this fit on one page" rests on it, so an estimate
    within a couple of lines is enough. Wrapped columns are the reason a row
    is not always one line: p-columns break their text, and a table split
    across a page boundary is a defect the module specification calls out.
    """
    fracs = [float(f) for f in
             re.findall(r"p\{([\d.]+)\\textwidth\}", spec)]
    char_pt = SIZE_CHAR.get(size, CHAR_PT)
    caps = [max(8.0, f * TEXT_PT / char_pt) for f in fracs]
    rows = 0.0
    for r in cells:
        if caps:
            rows += max([1] + [math.ceil(len(c) / caps[i])
                               for i, c in enumerate(r[:len(caps)])])
        else:
            rows += 1
    cap_lines = math.ceil(len(caption) / 88.0) if caption else 0
    # Rules, plus the vertical skips the surrounding center environment adds
    # at each end. Underestimating is the expensive direction: the table is
    # then placed with too little room and spills its last row.
    return SIZE_LINE.get(size, 1.0) * rows + cap_lines + 3.5
...
def table(rows: list[str], caption: str = "") -> str:
    ...
    out = []
    est = table_height(cells, spec, size, caption)
    if est <= KEEP_WHOLE_LINES:
        # Demand the whole table's height before starting it, so a short
        # table moves to the next page rather than straddling the boundary.
        out.append(r"\needspace{" + f"{est:.1f}" + r"\baselineskip}")
    out += [r"\begin{center}",
...
def figure_block(fname: str, caption: str) -> str:
    ...
    return "\n".join([
        ...
        # The height cap is what keeps a portrait figure off the footer.
        # qualitative_examples is 1.89 times as tall as it is wide, so at
        # 0.80\textwidth alone it stood 98% of the text height and its
        # five-line caption printed straight through the page number and the
        # student ID. keepaspectratio means the cap binds only on figures
        # tall enough to need it; every other figure here is landscape and
        # is still sized by the width.
        r"\includegraphics[width=0.80\textwidth,height=0.78\textheight,"
        r"keepaspectratio]{figures/" + safe + "}",
...
def signed_checklist() -> Path:
    """The countersigned Secondary Data Checklist, if it has been supplied.

    Untracked: it carries the supervisor's signature and the repository is
    public. Both the dissertation and the standalone ethics record bind it
    in when it is present, and say plainly on the page when it is not.
    """
    return SRC / "checklist_signed.pdf"
...
def write_zip(out: Path) -> None:
    ...
    for f in sorted(out.rglob("*")):
        ...
        if rel.parent == Path(".") and (f.suffix in BUILD_PRODUCTS
            ...
            continue
        # The countersigned checklist carries a signature and this zip is
        # tracked, so it would publish one. Overleaf gets the dissertation
        # without the form bound in; the submitted PDF is built locally,
        # where the file is.
        if rel.name == "checklist-signed.pdf":
            continue
        files.append((f, rel))
...
def build_stamp() -> str:
    """Commit and time of this build, for the PDF's Subject field.

    A downloaded PDF is indistinguishable from a current one by eye, which
    has repeatedly sent reviewers back to defects already fixed. This makes
    the build identifiable from Document Properties."""
    import subprocess
    ...
    if not h:
        h = "no-git"
    return (f"build {h}{'+dirty' if dirty else ''} "
...
def main():
    ...
    # 4.15 reports the volunteer study, and one version of the dissertation
    # does not carry it. Its table's caption would then be declared for a
    # table that is not there, which trips the registry check below on every
    # build. A warning that is always wrong teaches the reader to skip the
    # ones that are not, and the checklist warning above must not be skipped.
    ch4 = (SRC / "chapter4_results_rq1.md").read_text(encoding="utf-8")
    if "4.15 A disinterested check" not in ch4:
        TABLES["chapter4_results_rq1.md"] = TABLES["chapter4_results_rq1.md"][:-1]
    ...
    # The template asks for Highlights between the abstract and the
    # acknowledgements: 4-6 bullets, at most 100 characters each.
    ...
    if lead and chunks:
        chunks.insert(0, "\\chapter*{Supplementary Material}\n"
                      ...
                      + convert(lead, []))
    # The module requires the countersigned Secondary Data Checklist inside
    # the dissertation ("You include it in your MSc dissertation as part of
    # the Appendix"), not only in the ethics record submitted beside it. It
    # carries a signature, so it is deliberately untracked: put the exported
    # PDF at dissertation/checklist_signed.pdf and it binds itself in here.
    # Missing, the build says so in the log and on the page, because a
    # supplement that promises a form the reader cannot find is worse than
    # one that explains where it is.
    signed = signed_checklist()
    for i, c in enumerate(chunks):
        if not c.startswith("\\chapter{Ethical Approval}"):
            continue
        if signed.exists():
            shutil.copy2(signed, out / "figures" / "checklist-signed.pdf")
            chunks[i] = c + (
                "\n\n\\includepdf[pages=-]{figures/checklist-signed.pdf}\n")
        else:
            chunks[i] = c + (
                "\n\n\\medskip\\noindent\\textit{[The countersigned checklist "
                "binds in here from dissertation/checklist\\_signed.pdf, which is "
                "untracked because it carries a signature. It is not in this "
                "build.]}\n")
            print("  WARNING: dissertation/checklist_signed.pdf is missing, so "
                  "the signed\n           checklist is NOT bound into the "
                  "dissertation. Export it from\n           the .docx and "
    ...
    print("submit:   copy the compiled main.pdf to 6949963.pdf; the module "
          "asks for\n          the file to be named with the student ID.")
...
% Binds the countersigned Secondary Data Checklist in as page images. The
% module requires the signed form inside the dissertation, not only
% alongside it.
\usepackage{pdfpages}
% plainpages=false stops roman page i and arabic page 1 claiming the same
% PDF destination name, which made one internal link ambiguous.
...
\hypersetup{pdfsubject={__BUILDSTAMP__}}
...
% fancyhdr wants 13.6pt of header room for a 12pt header line and warns on
% every page otherwise, eighty times in this document. Giving it the room
% keeps the rule off the text and empties the log of a warning that would
% otherwise hide a real one.
