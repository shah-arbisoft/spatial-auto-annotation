...
A 4x-larger depth model moves front/behind recall by +0.000/-0.002 and mean recall by -0.002. The depth-predicate limit is monocular ambiguity - two objects at a similar distance are inseparable by *any* monocular model - not the depth network's fidelity. This is why the fix that worked (the ground-plane fallback, A7) is a geometric cue, not a bigger perception model. The shipped tool keeps the Small variant: identical accuracy, Apache-2.0 licence, half the VRAM.
